...
class LoaderUtil {
    ...
    func showLoading(size: CGSize? = nil, msg: String? = nil) {
        DispatchQueue.main.async {
            if self.isLoadingVisible() {
                LogHandler.reportLogOnConsole("Already loading view visible")
            } else {
                guard
    ...
    func noInternetConnection() {
        LoaderUtil.shared.showLoading(size: CGSize(width: UIScreen.main.bounds.width - 60, height: 50), msg: "no_internet_connection".localized())
